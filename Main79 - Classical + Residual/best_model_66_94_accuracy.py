...
CACHE_FILE = os.path.join(SAVE_DIR, "main66_final_pipeline.joblib")
...
def train_nbsvm(train_strings, y_train):
    ...
    return vectorizer, ratio, model


def predict_nbsvm(model, vectorizer, ratio, query_strings):
...
def local_geometry_features(
    ...
    nnA, nnB = fit_local_geometry_model(
        X_reference,
        y_reference,
        k=k
    )

    return local_geometry_from_neighbors(
        nnA,
        nnB,
        X_query
    )
...
def local_geometry_from_neighbors(
    nnA,
    nnB,
    X_query
):
    """
    Use already-fitted A/B nearest-neighbor models.
    Positive = A, negative = B.
    """
...
def save_final_pipeline(
    ...
    pipeline = {
        "tfidf": tfidf,
        "trans_vectorizer": trans_vectorizer,
        "compact_scaler": compact_scaler,
        "svm": svm,
        "nbsvm_vectorizer": nbsvm_vectorizer,
        "nbsvm_ratio": nbsvm_ratio,
        "nbsvm_model": nbsvm_model,
        "hgb": hgb,
        "nnA": nnA,
        "nnB": nnB,
    ...
    print("\nSaved final trained pipeline:")
    print(os.path.abspath(CACHE_FILE))


def load_final_pipeline():
    print("\nSaved model found.")
    print("Loading:", os.path.abspath(CACHE_FILE))
    ...
    if missing:
        raise RuntimeError(
            "Saved pipeline is incomplete. Missing: "
            + ", ".join(missing)
        )
...
def predict_with_saved_pipeline(test_texts, pipeline):
    (
        test_strings,
        test_transitions,
        X_test_tfidf,
        ...
        test_texts,
        pipeline["tfidf"],
        pipeline["trans_vectorizer"],
        pipeline["compact_scaler"]
    )

    test_svm = pipeline["svm"].decision_function(
        X_test_global
    )

    test_nbsvm = predict_nbsvm(
        pipeline["nbsvm_model"],
        pipeline["nbsvm_vectorizer"],
        pipeline["nbsvm_ratio"],
        test_strings
    )

    test_hgb = predict_hgb(
        pipeline["hgb"],
        X_test_compact
    )

    test_local = local_geometry_from_neighbors(
        pipeline["nnA"],
        pipeline["nnB"],
        X_test_tfidf
    )
    ...
    test_meta = make_meta_features(
        ...
        test_hgb,
        test_local,
        pipeline["meta_scaler"]
    )

    test_probability_B = pipeline["meta_model"].predict_proba(
        test_meta
    )[:, 1]
    ...
    return test_labels, test_probability_B
...
def main():
    ...
    # --------------------------------------------------------
    # If a final trained pipeline already exists, reuse it.
    # This skips all model training.
    # --------------------------------------------------------
    ...
    if os.path.exists(CACHE_FILE):
        ...
        print("\nPredicting with saved final pipeline...")
        test_labels, test_probability_B = predict_with_saved_pipeline(
            test_texts,
            pipeline
        )
        ...
        assert np.all(np.isin(test_labels, ["A", "B"]))
        assert np.all(np.isfinite(test_probability_B))
        ...
        print("\nSUBMISSION CREATED FROM SAVED MODEL")
    ...
    # --------------------------------------------------------
    # No saved final model: train the pipeline from scratch.
    # --------------------------------------------------------

    print("\nNo saved final model found.")
    print("Training a new final pipeline...")
    ...
    svm_val_model = train_svm(
        X_train_global,
        y_train
    )
    val_svm = svm_val_model.decision_function(
        X_val_global
    )

    print("Training validation NBSVM...")
    (
        nbsvm_val_vectorizer,
        nbsvm_val_ratio,
        nbsvm_val_model
    ) = train_nbsvm(
        train_strings,
    ...
    print("\nFinal SVM...")
    final_svm_model = train_svm(
        X_all_global,
        labels
    )
    test_svm = final_svm_model.decision_function(
        X_test_global
    )

    print("Final NBSVM...")
    (
        final_nbsvm_vectorizer,
        final_nbsvm_ratio,
        final_nbsvm_model
    ) = train_nbsvm(
        all_strings,
    ...
    final_nnA, final_nnB = fit_local_geometry_model(
        X_all_tfidf,
        labels,
        k=LOCAL_K
    )
    ...
    # Save every fitted object needed to reproduce the final model.
    save_final_pipeline(
        full_tfidf,
        full_trans_vectorizer,
        full_compact_scaler,
